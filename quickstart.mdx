...
A workflow is sequence of nodes, where at every node, some work is done. You can pass information from one node to the next by connecting them.

**Agent node:** The agent node is an AI model that can complete a piece of work. It requires a role, context and input. The 'role' is a job title that you want the agent to have. The 'context' is the background knowledge you would like the agent to have in its memory. Lastly, the 'input' is the prompt to the agent and is usually dynamically given by the previous node.

**Action node:** The action node is an automated action with no AI used. It is predicatble and predetermined. For example, the 'Excel - Detect Changes' action node will looky the connected excel file and output the changes in the same structured format every time.
